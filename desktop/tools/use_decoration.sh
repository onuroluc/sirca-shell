...
case "${1:-}" in
  glass) [ -f "$D/org.kde.glass19.so" ] || { echo "install first:  sudo $(cd "$(dirname "$0")" && pwd)/install_qt.sh"; exit 1; }
         ...
         kwriteconfig6 --file kwinrc --group org.kde.kdecoration2 --key library org.kde.glass19; kwriteconfig6 --file kwinrc --group org.kde.kdecoration2 --key theme Glass ;;
